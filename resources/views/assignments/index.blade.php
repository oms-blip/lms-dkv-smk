<x-app-layout>
    <div x-data="{ activeTab: 'aktif', searchQuery: '' }" class="min-h-screen bg-slate-50">
            ...
            <div class="flex items-center justify-between w-full">
                ...
                <div class="flex items-center space-x-4">
                    <div class="relative" x-data="{ openNotif: false }">
                        @php $jumlahNotif = 0; @endphp <button @click="openNotif = !openNotif" @click.away="openNotif = false" class="relative text-slate-400 hover:text-blue-600 transition-colors p-2 rounded-full hover:bg-slate-100 outline-none">
        ...
        <div class="p-8">
            <div class="max-w-7xl mx-auto space-y-8">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h3 class="text-xl font-bold text-slate-800">Kelola Tugas</h3>
                        <p class="text-xs text-slate-400 mt-0.5">Buat tugas baru, lihat pengumpulan, dan berikan penilaian.</p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-4">
                        ...
                        <div><p class="text-2xl font-black text-slate-800">{{ $totalTugas ?? 0 }}</p><p class="text-xs font-medium text-slate-400">Total Tugas Buatan</p></div>
                    </div>
                    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-4">
                        ...
                        <div><p class="text-2xl font-black text-slate-800">{{ $menungguDinilai ?? 0 }}</p><p class="text-xs font-medium text-slate-400">Menunggu Dinilai</p></div>
                    </div>
                    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-4">
                        ...
                        <div><p class="text-2xl font-black text-slate-800">{{ $selesaiDinilai ?? 0 }}</p><p class="text-xs font-medium text-slate-400">Tugas Selesai Dinilai</p></div>
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-3 border-b border-slate-100 pb-2">
                    ...
                    <button @click="activeTab = 'dinilai'" :class="activeTab === 'dinilai' ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'" class="px-5 py-2 rounded-xl text-xs font-bold transition-all outline-none">
                        Sudah Dinilai
                    </button>
                </div>
                ...
                <div class="space-y-4">
                    @php
                        // Menampung koleksi tugas dari database (jika kosong, status aman terkendali)
                    ...
                    @if($totalTugasCount > 0)
                        @foreach($daftarTugas as $task)
                            @php
                                // Mengatur penanda status internal untuk filter Alpine
                                $statusTugas = ($task->submissions_count == $task->graded_count && $task->submissions_count > 0) ? 'dinilai' : 'aktif';
                            @endphp
                            ...
                            <div x-data="{ title: '{{ strtolower($task->title) }}', status: '{{ $statusTugas }}' }"
                                ...
                                <div>
                                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-md {{ $statusTugas === 'aktif' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600' }} uppercase mb-2 inline-block">
                                        {{ $statusTugas === 'aktif' ? 'Aktif / Berjalan' : 'Selesai Dinilai' }}
                                    </span>
                                    <h4 class="font-bold text-slate-800 text-base">{{ $task->title }}</h4>
                                    <p class="text-xs text-slate-400 mt-1">Batas Pengumpulan: {{ \Carbon\Carbon::parse($task->due_date)->format('d M Y, H:i') }} WIB</p>
                                </div>
                    ...
                    <div x-show="Array.from($el.parentNode.children).filter(c => c.classList.contains('bg-white') && c.style.display !== 'none').length === 0" 
                         class="text-center py-16 bg-white rounded-2xl border border-slate-200 border-dashed">
